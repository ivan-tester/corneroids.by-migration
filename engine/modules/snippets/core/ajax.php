<?php
if(!defined('ROOT_DIR')) {define('ROOT_DIR', $_SERVER['DOCUMENT_ROOT']);define('ENGINE_DIR', ROOT_DIR.'/engine');}
...
if ($config['http_home_url'] == "") {$config['http_home_url'] = "http://".$_SERVER['HTTP_HOST'].reset(explode("engine/ajax/pages.php", $_SERVER['PHP_SELF']));}

require_once ENGINE_DIR.'/classes/mysql.php';
require_once ENGINE_DIR.'/data/dbconfig.php';

$PHP_SELF = $config['http_home_url']."index.php";
$_TIME = time()+(intval( $config['date_adjust'] ) * 60);
...
foreach ($cat_info as $cats) if ($cats['skin'] != '') $allow_sql_skin = true;
...
if ($category_skin != "") if (@is_dir(ROOT_DIR.'/templates/'.$category_skin)) $config['skin'] = $category_skin;
elseif ($_COOKIE['dle_skin'] != '') if (@is_dir(ROOT_DIR.'/templates/'.$_COOKIE['dle_skin'])) $config['skin'] = $_COOKIE['dle_skin'];

if ($config["lang_".$config['skin']]) require_once ROOT_DIR.'/language/'.$config["lang_".$config['skin']].'/website.lng';
else require_once ROOT_DIR.'/language/'.$config['langs'].'/website.lng';
