...
if (!empty($_SERVER['HTTP_REFERER']) AND clean_url($_SERVER['HTTP_REFERER']) != clean_url($_SERVER['HTTP_HOST'])) die("Hacking attempt!");
...
class genrandomimage {
	...
	function genimage() {
		...
		if( !function_exists('imagecreatetruecolor') OR !function_exists('imagecreatefrompng') ) {
			header( "Expires: Tue, 11 Jun 1985 05:00:00 GMT" );
			header( "Cache-Control: no-store, no-cache, must-revalidate" );
			header( "Pragma: no-cache" );
			header( "Content-Type: image/svg+xml" );
			echo '<svg xmlns="http://www.w3.org/2000/svg" width="120" height="50"><rect width="120" height="50" fill="#eef4f4"/><text x="60" y="32" text-anchor="middle" font-family="Arial, sans-serif" font-size="22" font-weight="bold" fill="#243438">'.$this->keystring.'</text></svg>';
			return;
		}
		...
		while (true) {
			...
			for ($i = 0; $i < $this->length; $i++) {
				...
				$y = (int)round( mt_rand( -$this->fluctuation_amplitude, $this->fluctuation_amplitude ) + ($this->height - $fontfile_height) / 2 + 2 );
		...
		for ($x = 0; $x < $this->width; $x++) {
			for ($y = 0; $y < $this->height; $y++) {
				...
				if ($sx < 0 || $sy < 0 || $sx >= $this->width - 1 || $sy >= $this->height - 1) {
					...
					$sx_int = (int)floor( $sx );
					$sy_int = (int)floor( $sy );
					$color = imagecolorat( $img, $sx_int, $sy_int ) & 0xFF;
					$color_x = imagecolorat( $img, $sx_int + 1, $sy_int ) & 0xFF;
					$color_y = imagecolorat( $img, $sx_int, $sy_int + 1 ) & 0xFF;
					$color_xy = imagecolorat( $img, $sx_int + 1, $sy_int + 1 ) & 0xFF;
				}
				...
				imagesetpixel( $img2, $x, $y, imagecolorallocate( $img2, (int)round( $newred ), (int)round( $newgreen ), (int)round( $newblue ) ) );
			}
		}
