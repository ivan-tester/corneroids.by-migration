...
class KCAPTCHA{
	...
	function __construct(){
		...
		if( !function_exists('imagecreatetruecolor') OR !function_exists('imagecreatefrompng') OR !count($fonts) ) {
			while(true){
				$this->keystring='';
				for($i=0;$i<$length;$i++){
			...
			echo '<svg xmlns="http://www.w3.org/2000/svg" width="160" height="80"><rect width="160" height="80" fill="#eef4f4"/><text x="80" y="48" text-anchor="middle" font-family="Arial, sans-serif" font-size="28" font-weight="bold" fill="#243438">'.$this->keystring.'</text></svg>';
			return;
		}
		...
		do{
			...
			for($i=0;$i<$length;$i++){
				...
				$y = (int)round((($i%2)*$fluctuation_amplitude - $fluctuation_amplitude/2)*$odd
