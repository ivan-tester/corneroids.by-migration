...
if( $member_id['user_group'] > 2 and intval( $chat_cfg['stop_flood'] ) and ! $error) {
		$this_time = time() + (intval( $config['date_adjust'] ) * 60) - $chat_cfg['stop_flood'];
 ...
 if( $user_group[$member_id['user_group']]['allow_url'] ){
	...
	function iChat_build_url($url = array()) {
		...
		if( iChat_check_home( $url['html'] ) OR $url['html'][0] == "/" OR $url['html'][0] == "#") $target = "";
			...
			$message = preg_replace_callback( "#\[leech\](\S.+?)\[/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[1])); }, $message );
			$message = preg_replace_callback( "#\[leech\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[2])); }, $message );
			$message = preg_replace_callback( "#\[leech\s*=\s*(\S.+?)\s*\](.*?)\[\/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[2])); }, $message );
