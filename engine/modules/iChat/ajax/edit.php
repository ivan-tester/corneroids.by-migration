...
$message = sqlite_fetch_array(sqlite_query( $iChat_db, "SELECT message FROM iChat WHERE id = '$id'" ));
	...
	function iChat_decode_leech($url = "", $show = "") {
		...
		$message = preg_replace_callback( "#<!--dle_leech_begin--><a href=[\"'](http://|https://|ftp://|ed2k://|news://|magnet:)?(\S.+?)['\"].*?" . ">(.*?)</a><!--dle_leech_end-->#i", function($matches) { return iChat_decode_leech($matches[1] . $matches[2], $matches[3]); }, $message );
...
if( $user_group[$member_id['user_group']]['allow_url'] ){
	...
	function iChat_build_url($url = array()) {
		...
		if( iChat_check_home( $url['html'] ) OR $url['html'][0] == "/" OR $url['html'][0] == "#") $target = "";
			...
			$new_message = preg_replace_callback( "#\[leech\](\S.+?)\[/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[1])); }, $new_message );
			$new_message = preg_replace_callback( "#\[leech\s*=\s*\&quot\;\s*(\S+?)\s*\&quot\;\s*\](.*?)\[\/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[2])); }, $new_message );
			$new_message = preg_replace_callback( "#\[leech\s*=\s*(\S.+?)\s*\](.*?)\[\/leech\]#i", function($matches) { return iChat_build_url(array('html' => $matches[1], 'show' => $matches[2])); }, $new_message );
