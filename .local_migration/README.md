...
## 55. Обновление статуса: DLE 17.0 UTF-8 от 2026-06-27

Локальный UTF-8 стенд успешно обновлён с DLE 16.1 до DLE 17.0.

Использованный архив:

```text
/mnt/z/Ванина папка/1САЙТ/Движки/DLE/ЛИЦЕНЗИЯ/dle17_0.zip
```

Перед применением было проверено:

```text
install.php: version_id = 17.0
install.php: charset = utf-8
engine/inc/upgrade/16.1.php exists
```

Официальный changelog для Default template на переходе 16.1 -> 17.0 требует:

```text
1. Добавить CSS .mce-accordion summary.
2. Добавить CSS .self_delete_link.
3. В userinfo.tpl добавить тег [delete]Удалить аккаунт[/delete] в секции редактирования профиля.
```
...
```text
.local_migration/patches/pisces-template-16.1-to-17.0.patch
```
...
```text
.local_migration/www_utf8/engine/data/config.php: version_id = 17.0
charset = utf-8
skin = Pisces
install.php удалён
```
...
```text
.local_migration/checkpoints/17.0-utf8-php74-ok
```
...
```text
Подготовить upgrade DLE 17.0 UTF-8 -> DLE 17.1.
Перед применением проверить charset/version в официальном архиве dle17_1.zip и изменения шаблона 17.0 -> 17.1.
```

## 56. Обновление статуса: DLE 17.1 UTF-8 от 2026-06-27

Локальный UTF-8 стенд успешно обновлён с DLE 17.0 до DLE 17.1.

Использованный архив:

```text
/mnt/z/Ванина папка/1САЙТ/Движки/DLE/ЛИЦЕНЗИЯ/dle17_1.zip
```

Перед применением было проверено:

```text
install.php: version_id = 17.1
install.php: charset = utf-8
engine/inc/upgrade/17.0.php exists
```

Официальный changelog для Default template на переходе 17.0 -> 17.1 требует добавить CSS `DLEPush` и `comments_author_field`.
...
```text
.local_migration/patches/pisces-template-17.0-to-17.1.patch
```
...
```json
{"status": "ok", "version":"17.1"}
```

После upgrade:

```text
.local_migration/www_utf8/engine/data/config.php: version_id = 17.1
charset = utf-8
skin = Pisces
install.php удалён
```
...
```text
.local_migration/checkpoints/17.1-utf8-php74-ok
```
...
```text
Подготовить upgrade DLE 17.1 UTF-8 -> DLE 17.2.
По changelog 17.1 -> 17.2 изменений в шаблонах не требуется.
```

## 57. Обновление статуса: DLE 17.2 UTF-8 от 2026-06-27

Локальный UTF-8 стенд успешно обновлён с DLE 17.1 до DLE 17.2.

Использованный архив:

```text
/mnt/z/Ванина папка/1САЙТ/Движки/DLE/ЛИЦЕНЗИЯ/dle17_2.zip
```

Перед применением было проверено:

```text
install.php: version_id = 17.2
install.php: charset = utf-8
engine/inc/upgrade/17.1.php exists
```

Официальный changelog:

```text
17.1 -> 17.2: изменений в шаблонах не требуется.
```
...
```text
.local_migration/www_utf8/engine/data/config.php: version_id = 17.2
charset = utf-8
skin = Pisces
install.php удалён
```
...
```text
.local_migration/checkpoints/17.2-utf8-php74-ok
```
...
```text
Подготовить upgrade DLE 17.2 UTF-8 -> DLE 17.3.
...
```text
DLE 19.0 UTF-8 -> DLE 19.1 выполнен, см. раздел 62.
```

## 62. Обновление статуса: DLE 19.1 UTF-8 от 2026-06-27

Миграция `19.0 -> 19.1` выполнена на UTF-8 копии:
...
```text
/mnt/z/Ванина папка/1САЙТ/Движки/DLE/ЛИЦЕНЗИЯ/dle19_1.zip
```

Распакованная копия:

```text
.local_migration/dle_versions/19.1/extracted
```

Проверки архива перед применением:

```text
install.php: version_id = 19.1
install.php: charset = utf-8
engine/inc/upgrade/19.0.php найден
```

Файлы DLE 19.1 наложены поверх активной UTF-8 копии с сохранением пользовательских данных и шаблона:

```bash
rsync -a --no-owner --no-group --omit-dir-times --delete \
  --exclude='/templates/' --exclude='/engine/data/' --exclude='/engine/cache/' \
  --exclude='/uploads/' --exclude='/backup/' \
  .local_migration/dle_versions/19.1/extracted/upload/ \
  .local_migration/www_utf8/
```

Upgrade-мастер успешно прошёл шаг `19.0 -> 19.1` под локальным migration-admin `Personage`:

```text
ajax upgrade response: {"status":"ok","version":"19.1"}
admin upgrade check: Обновление скрипта до версии 19.1 завершено.
```

Финальное состояние:

```text
.local_migration/www_utf8/engine/data/config.php: version_id = 19.1
charset = utf-8
skin = Pisces
install.php удалён
```
...
```text
.local_migration/checkpoints/19.1-utf8-php74-ok
```

### 62.1. Изменения шаблона Pisces для DLE 19.1
...
```text
templates/Pisces/style/engine.css
- добавлены CSS-правила для .smart-captcha, mathml и .dleshowhidden.

templates/Pisces/attachment.tpl
- ссылка {online-view-link} получила data-highslide="file" и data-type="pdf".
```

Patch с адаптацией шаблона:

```text
.local_migration/patches/pisces-template-19.0-to-19.1.patch
```

### 62.2. Проверка после DLE 19.1
