...
require_once(LB_CLASS . '/HTMLSax3.php');
...
class SafeHTML 
{
     ...
     * @access public
     */
    function parse($doc, $nl2br = false, $LB_specchar = false) 
    {
       $this->clear();
       ...
       if($nl2br)
       {
            ...
            $this->_xhtml = preg_replace( $find, $replace, $this->_xhtml );
       }
    ...
    function LB_specchar () 
    {
	   $this->_xhtml = strtolower($this->_xhtml);
	   $quotes = array ("\x27", "\x22", "\x60", "\t", "\n", "\r", "'", ",", "/", "¬", ";", "@", "~", "[", "]", "{", "}", "=", ")", "(", "*", "&", "^", "%", "$", "<", ">", "?", "!", '"' );
	   $this->_xhtml = str_replace( $quotes, '', $this->_xhtml );
