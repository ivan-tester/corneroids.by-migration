...
function bb_decode($msg, $bb_allowed)
{
    ...
    if (!count($bb_allowed) OR (count($bb_allowed) AND in_array("youtube", $bb_allowed)))
        $msg = preg_replace_callback("#\[youtube=(\S.+?)\]#is".regular_coding(), function($matches) { return bb_create_video($matches[1]); }, $msg);
    ...
    $msg = preg_replace_callback("#<a href=['\"](\S.+?)['\"]\s*(target=\"_blank\")?\s*>(.+?)</a>#is".regular_coding(), function($matches) { return bb_url($matches[1], $matches[3], false); }, $msg); //url
    }
    ...
    {
        $msg = preg_replace_callback("#\[img=?\](.+?)\[/img\]#is".regular_coding(), function($matches) { return bb_create_img($matches[1]); }, $msg); //img center
    	$msg = preg_replace_callback("#\\[img=(left|right|center)+?\\](.+?)\\[/img\\]#is".regular_coding(), function($matches) { return bb_create_img($matches[2], $matches[1]); }, $msg); //img left|right
    }
...
function bb_encode($msg)
{
    ...
    $pattern = array(
                    "#<blockquote class=\"blockspoiler\"><span class=\"titlespoiler\"><a href='\#' onclick=\"ShowAndHide\('.+?'\); return false;\">".$spoiler_title."</a></span><div id='.+?' style='display:none;' class=\"textspoiler\">#is".regular_coding(),
                    "#<blockquote class=\"blockspoiler\"><span class=\"titlespoiler\"><a href='\#' onclick=\"ShowAndHide\('.+?'\); return false;\">(.+?)</a></span><div id='.+?' style='display:none;' class=\"textspoiler\">#is".regular_coding(), 
    ...
    $msg = preg_replace('#<iframe src="http://vkontakte.ru/video_ext.php\?(.*?)" width="([0-9]){1,4}" height="([0-9]){1,4}" frameborder="0"></iframe>#is'.regular_coding(), '[youtube=http://vkontakte.ru/video_ext.php?\\1]', $msg); //youtube

    $msg = preg_replace_callback("#<!-- Small_img:([:a-z_\-/\.0-9]+?)\|(left|right|center)? -->(.+?)<!--/Small_img -->#is".regular_coding(), function($matches) { return bb_create_img_back($matches[1] . '|' . $matches[2]); }, $msg); //php	
...
function bb_create_video ($url = "")
{
    ...
    $allowed_host = array("youtube.com", "youtu.be", "rutube.ru", "video.rutube.ru", "vkontakte.ru");
    ...
    elseif ($url_parse['host'] == "vkontakte.ru")
    {
        $query = explode('&', $url_parse['query']);
        $massiv = array();
        foreach ($query as $value)
        ...
        $buld_code = '<iframe src="http://vkontakte.ru/video_ext.php?'.$video_link.'" width="607" height="360" frameborder="0"></iframe>';    
    }
    return $buld_code;
}
