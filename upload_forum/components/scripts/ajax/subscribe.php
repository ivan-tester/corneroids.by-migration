...
{
    $lang_s_a_subscribe = language_forum ("board/scripts/ajax/subscribe");
    ...
    if (!$topic['id'])
        echo show_jq_message ("3", $lang_s_a_subscribe['error_title'], $lang_s_a_subscribe['error_info']);
    else
    {
        include LB_CLASS.'/topics_out.php';
        $LB_topics = new LB_topics;
        $LB_topics->subscribe ($topic['id'], $topic['forum_id'], true, $topic['status'], $topic['hiden'], true );
        unset($LB_topics);
    }
