...
{  
    ...
    if ($type_sort != 7) $tpl->result['block_news'] = $cache->take("dle_news_".$type_sort, "", "dle_modules", 1);
    ...
    if( !$tpl->result['block_news'] )
    {
        ...
        $DB->prefix = DLE_PREFIX;
        $DB->select( "id, title, date, alt_name, category, flag, short_story, xfields", "post", $where, $order." LIMIT 0,".$num );
        ...
        if ($type_sort != 7) $cache->take("dle_news_".$type_sort, $tpl->result['block_news'], "dle_modules");
    }
